...
public class RandomValueGenerator
{
    ...
    private static readonly Dictionary<Type, Func<object>> RandomValueGenerators = new()
    {
        ...
        { typeof(float), () => CreateRandomFloat() },
        { typeof(float?), () => (float?)CreateRandomFloat() },
    ...
    public static object? CreateRandomValue(Type objectType)
    {
        ...
        if (objectType.IsGenericType && objectType.GetGenericTypeDefinition() == typeof(Nullable<>))
        {
            ...
            {
                throw new Exception($"Failed to determine the underlying Type for [{objectType.Name}]");
            }

            return CreateRandomValue(underlyingType);
        }
        ...
        if (randomType is { IsEnum: true })
        {
            return CreateRandomEnum(randomType);
        }

        if (randomType is { IsGenericType: true })
        {
        ...
        if (randomType is { IsArray: true, IsClass: true })
        {
            var elementType = randomType.GetElementType();
            if (elementType == null)
            {
                throw new Exception($"Failed to retrieve the Element Type for {randomType.Name}");
            }

            return CreateRandomArray(elementType);
        }
    ...
    public static float CreateRandomFloat(long minimumValue = long.MinValue, long maximumValue = long.MaxValue)
    {
        float randomNumber = CreateRandomNumber((int)minimumValue, (int)maximumValue);
    ...
    public static object? CreateRandomEnum(Type enumType)
    {
        ...
        IEnumerable<object?> possible = Enum.GetValues(enumType).Cast<object>();
    ...
    private static T? CreateRandomFrom<T>(IEnumerable<T> items)
    {
        ...
        return maxValue <= 0
                   ? itemArray.First()
                   : itemArray.Skip(CreateRandomInt(0, maxValue)).First();
    }
    ...
    {
        var listType         = typeof(List<>).MakeGenericType(genericCollectionType);
    ...
    {
        var listType         = typeof(Collection<>).MakeGenericType(genericCollectionType);
        var randomCollection = (IList)Activator.CreateInstance(listType)!;
        var howMany          = CreateRandomInt(minItems, maxItems);
    ...
    public static dynamic CreateRandomDictionary(Type objectType, int minItems = MinimumCollectionItems, int maxItems = MaximumCollectionItems)
    {
        ...
        for (var loopCount = 0; loopCount < howMany; loopCount++)
        {
            var keyValue = CreateRandomValue(genericArguments[0]);
            if (keyValue == null || randomDictionary.Contains(keyValue))
            {
                continue;
            }

            var dictionaryValue = CreateRandomValue(genericArguments[1]);
    ...
    private static DateTime RangeCheckTimeOnRandomDate(DateTime? minTime, DateTime? maxTime, DateTime value)
    {
        ...
        if (!(minTime > maxTime))
        {
            return value > maxTime || value < minTime
                       ? CreateRandomDate(minTime, maxTime)
                       : value;
        }
